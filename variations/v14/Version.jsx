// V14 — V12 base · Primary nav in top header · Operational data above company info in drawer
...
const STATUS_CHIPS = [
  { label: 'Cycle',     value: 'Mar 26 (125)',  fullValue: 'Mar 26 (125)',                             accent: true,  dim: false },
  { label: 'Status',    value: 'Active',         fullValue: 'Active',                                  accent: true,  dim: false },
  { label: 'Plan',      value: 'RH+',            fullValue: 'RH+',                                     accent: false, dim: false },
  { label: 'Employees', value: '7',              fullValue: '7',                                        accent: false, dim: false },
  { label: 'Usage',     value: 'Client',         fullValue: 'Client',                                   accent: false, dim: true  },
  { label: 'Origin',    value: 'Migration',      fullValue: 'Migration',                                accent: false, dim: true  },
...
// ── Top header: identity · primary nav · login ────────────────────────────────

function TopHeader({ currentCompany, onCompanyChange, currentPage, onPageChange }) {
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'stretch', borderBottom: '1px solid var(--grey-200)', background: 'var(--white)', flexShrink: 0, minHeight: 56, position: 'relative' }}>

      {/* Identity */}
      <div style={{ display: 'flex', flexDirection: 'column', justifyContent: 'center', gap: 3, padding: '10px 16px', alignSelf: 'stretch', position: 'relative', flexShrink: 0 }}>
        <button
      ...
      {/* Secondary nav — pinned right */}
      <div style={{ display: 'flex', alignItems: 'stretch', flexShrink: 0 }}>
        <div style={{ width: 1, height: 20, background: 'var(--grey-200)', alignSelf: 'center', marginRight: '4px' }} />
      ...
      {/* Login — pinned right */}
      <div style={{ padding: '0 16px', flexShrink: 0, display: 'flex', alignItems: 'center', borderLeft: '1px solid var(--grey-200)' }}>
...
// ── Data drawer: operational metadata + collapsible sections ──────────────────
...
function OperationalRow({ label, value, fullValue, accent, dim, mono }) {
  const { hovered, setHovered, copied, handleClick } = useCopyable(fullValue)
  return (
    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '5px 12px' }}>
      ...
      >
        ...
        <span style={{
          ...
          background: hovered
            ? (accent ? '#d9e3ff' : dim ? 'var(--grey-100)' : 'var(--grey-200)')
            : (accent ? '#EBF0FF' : dim ? 'transparent' : 'var(--grey-100)'),
...
function ContextPanel() {
  return (
    <div style={{ width: '240px', flexShrink: 0, borderRight: '1px solid var(--grey-200)', display: 'flex', flexDirection: 'column', background: 'var(--white)', overflowY: 'auto' }}>

      {/* Operational metadata */}
      <div style={{ padding: '4px 0', borderBottom: '1px solid var(--grey-200)', display: 'flex', flexDirection: 'column', flexShrink: 0 }}>
        {STATUS_CHIPS.map(({ label, value, fullValue, accent, dim, mono }) => (
          <OperationalRow key={label} label={label} value={value} fullValue={fullValue} accent={accent} dim={dim} mono={mono} />
        ))}
      </div>
